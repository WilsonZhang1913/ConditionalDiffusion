...
class AllWeather:
    ...
    def get_loaders(self, parse_patches=False, validation='snow'):
        if validation == 'raindrop':
            ...
            path = os.path.join(self.config.data.data_dir, 'data', 'allweather')
            filename = 'allweather_test.txt'
...
class AllWeatherDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True):
        ...
        with open(train_list) as f:
            ...
            #img_ids = [re.split('/', input_name)[-1].replace('rain', 'clean') for input_name in input_names]
            # gt_names += [os.path.join(os.path.join(raindrop_dir, 'gt'), i.replace('rain', 'clean')) for i in images]
            gt_names = [i.strip().replace('data', 'gt').replace('rain', 'clean').replace('tclean', 'train') for i in input_names]
            print(gt_names[0])
    ...
    def get_images(self, index):
        ...
        if self.parse_patches:
            ...
            # print(input_img.size, gt_img.size, wd_new, ht_new)
